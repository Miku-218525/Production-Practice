...
void CustomPlotForm::plot(const QVector<double> &vec_x, const QVector<double> &vec_y)
{
    ui->customplotwidget->addGraph();//添加曲线
    ui->customplotwidget->graph(0)->setData(vec_x,vec_y);//给曲线添加数据
    ui->customplotwidget->rescaleAxes();//自适应大小
    ui->customplotwidget->replot(QCustomPlot::rpQueuedReplot);
    //ui->customplotwidget->xAxis->setRange(0, 50, Qt::AlignLeft);
    //ui->customplotwidget->yAxis->setRange(0, 50, Qt::AlignLeft);
}
